    ...
    describe("by value", () => {
        ...
        describe("array destructuring", () => {
            ...
            it("should set values properly with setValue", async () => {
                ...
                expect(result.current[0]).toBe(initialState.age + 10);

                let currAge = result.current[0];

                act(() => {
                    setValue((age) => age + 20);
                });
                expect(result.current[0]).toBe(currAge + 20);

                currAge = result.current[0];
    ...
    describe("by state", () => {
        ...
        describe("array destructuring", () => {
            ...
            it("should set state properly with setState", async () => {
                ...
                // subscribe to age
                await act(async () => {
                    (() => result.current[0].age)();
                });

                let currProxyState = result.current[0];

                act(() => {
                    setState({ age: currProxyState.age + 10 });
                });

                expect(result.current[0].age).toBe(currProxyState.age + 10);
                currProxyState = result.current[0];

                act(() => {
                    setState((state) => ({ age: state.age + 20 }));
                });

                expect(result.current[0].age).toBe(currProxyState.age + 20);
                expect(result.current[0]).not.toBe(currProxyState);
            });
            it("should render only when subscribed value changes", async () => {
                const { result } = renderUseSubscribeState();
                const [, setState] = result.current;
                ...
                act(() => {
                    ...
                    expect(user.name.first).toBe("Miles");
                    expect(result.current[0].user.name.first).not.toBe("Miles");
                    expect(result.current[0]).toMatchObject(currProxyState);
                });
            });
            it("should render only for subscribed nested value changes", async () => {
                const { result } = renderUseSubscribeState();
                ...
                // subscribe to user.name.first
                await act(async () => {
                    (() => result.current[0].user.name.first)();
                });
                ...
                expect(result.current[0].user.name.first).toBe("Miles");
                ...
                act(() => {
                    expect(result.current[0].user.name.last).toBe("Parker");
                });
            });
        });
    });
